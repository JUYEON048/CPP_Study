...
void *thread_routine(void *arg) {
  ...
  for (int i = 0; i < 1000000; ++i) {
    //------>
    pthread_mutex_lock(&g_mutex);  // --> lock 1000000번

    sum += 1;

    pthread_mutex_unlock(&g_mutex);
    //<------ 임계 영역(Critical Section)
    //  :  오직 한개의 스레드만 메모리에 접근 가능하도록 해야 합니다.
    //    - 상호 배제(MUTual EXclusiton)
    //      : Mutex


    // sum 결과가 제대로 나오지 않는 원인 -- solution is "Criticla Section"
...
void *thread_routine(void *arg) {
  ...
  pthread_mutex_lock(&g_mutex); // --> lock 1번  >> 성능 향상
  sum += local;
  pthread_mutex_unlock(&g_mutex);
...
int main() {
  ...
  printf("sum: %d\n", sum);
  // sum 결과가 제대로 나오지 않는 이유를 이해하는 것이 중요합니다.
